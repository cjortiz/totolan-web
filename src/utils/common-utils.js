 ...
 */
export const checkIfFilterHasChanges = (filterInitialValues, currentFilterValues) => {
    let changesCount = 0;
    const changedKeys = [];
    const keys1 = filterInitialValues
    ...
    for (const key of keys1) {
        if (currentFilterValues &&
            ...
            if (currentFilterValues[key] !== undefined &&
                !(Array.isArray(currentFilterValues[key]) &&
                    currentFilterValues[key].length === 0)) {
                if (moment.isMoment(currentFilterValues[key]) &&
                    moment.isMoment(currentFilterValues[key])) {
                    const currentDate = currentFilterValues[key].format(DateFormat.MMDDYYYY);
                    const filterDate = filterInitialValues[key].format(DateFormat.MMDDYYYY);
                    if (filterDate !== currentDate) {
                        changesCount++;
                        changedKeys.push(key);
                    }
                }
                else if (typeof filterInitialValues[key] === "object" &&
                    typeof currentFilterValues[key] === "object") {
                    if (Array.isArray(filterInitialValues[key]) &&
                        Array.isArray(currentFilterValues[key])) {
                        if (currentFilterValues[key].length > 0) {
                            ...
                            const arrayChangesResult = checkIfFilterArrayHasChanges(sortedDefaultArray, sortedFormArray);
                        ...
                        const formChangesResult = checkIfFilterHasChanges(filterInitialValues[key], currentFilterValues[key]);
                    ...
                    if (filterInitialValues[key] instanceof Date &&
                        currentFilterValues[key] instanceof Date) {
                        if (!moment(filterInitialValues[key]).isSame(currentFilterValues[key])) {
                            changesCount++;
                            changedKeys.push(key);
                        }
                    }
 ...
 * @returns changesCount and changes keys
 */
const checkIfFilterArrayHasChanges = (defaultArrayValues, currentArrayValues) => {
    ...
    for (let i = 0; i < defaultArrayValues.length; i++) {
        if (typeof defaultArrayValues[i] === "object" &&
            typeof currentArrayValues[i] === "object") {
            const elementChangesResult = checkIfFilterHasChanges(defaultArrayValues[i], currentArrayValues[i]);
            ...
            if (defaultArrayValues[i] instanceof Date &&
 ...
 */
export const commonPaginationChangeHandler = (page, pageSize, filter, setFilter) => {
    setFilter({
        ...filter,
        pageNumber: page,
        pageSize: pageSize,
